...
namespace Common
{
    ...
    public partial struct NpcTargetingSystem : ISystem
    {
        ...
        public partial struct NpcTargetingJob : IJobEntity
        {
            ...
            [BurstCompile]
            public void Execute(Entity searchingEntity, ref NpcTargetEntity targetEntity,
                in LocalTransform transform, in NpcTargetRadius searchRadius)
            {
                ...
                if (CollisionWorld.OverlapSphere(transform.Position, searchRadius.Value, ref hits, CollisionFilter))
                {
                    ...
                    foreach (var hit in hits)
                    {
                        ...
                        if (MobaTeamLookup[searchingEntity].Value == mobaTeam.Value) { continue; }
